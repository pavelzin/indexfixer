...
// Renderuj monitor limitów API
$quota_monitor = IndexFixer_Quota_Monitor::get_instance();
$quota_stats = $quota_monitor->get_usage_stats();
$quota_history = $quota_monitor->get_usage_history(7);
$quota_estimation = $quota_monitor->estimate_quota_exhaustion();
?>
...
<?php
// Renderuj monitor limitów API na górze
$status_class = '';
$status_text = '';
$status_icon = '';

switch ($quota_stats['status']) {
    case 'exceeded':
...
<div class="notice <?php echo $status_class; ?>" style="margin-bottom: 20px;">
    ...
    <div style="display: flex; gap: 30px; align-items: center; margin: 15px 0; flex-wrap: wrap;">
        <div>
            <strong>Status:</strong> <?php echo $status_icon . ' ' . $status_text; ?>
        </div>
        <div>
            <strong>Dzisiaj:</strong> 
            <?php echo $quota_stats['daily']['used']; ?>/<?php echo $quota_stats['daily']['limit']; ?> 
            (<?php echo $quota_stats['daily']['percentage']; ?>%)
        </div>
        <div>
            <strong>Pozostało:</strong> <?php echo $quota_stats['daily']['remaining']; ?> requestów
        </div>
        <div>
            <strong>Ta minuta:</strong> 
            <?php echo $quota_stats['minute']['used']; ?>/<?php echo $quota_stats['minute']['limit']; ?>
        </div>
    </div>
    
    <?php if ($quota_stats['status'] !== 'ok'): ?>
        <div style="margin: 10px 0; padding: 10px; background: rgba(255,255,255,0.8); border-radius: 4px;">
            <strong>⚠️ Uwaga:</strong> 
            <?php if ($quota_stats['status'] === 'exceeded'): ?>
                Dzienny limit API został przekroczony. Sprawdzanie URL-ów zostało wstrzymane do jutra.
            <?php elseif ($quota_stats['status'] === 'critical'): ?>
    ...
    <div style="margin-top: 15px;">
        <strong>Prognoza:</strong> <?php echo htmlspecialchars($quota_estimation); ?>
    </div>
    ...
    $reset_time_server = $tomorrow_midnight_pacific->setTimezone($server_timezone);
    $time_until_reset = $tomorrow_midnight_pacific->getTimestamp() - time();
    $hours_until_reset = round($time_until_reset / 3600, 1);
    ?>
    
    <div style="margin-top: 10px; font-size: 12px; color: #666;">
        <strong>Następny reset limitów:</strong> 
        <?php echo $reset_time_server->format('Y-m-d H:i:s T'); ?> 
        (za <?php echo $hours_until_reset; ?> godzin)
    </div>
...
<div class="wrap">
    ...
    <div class="indexfixer-stats-boxes" style="background: #fff; padding: 24px; border-radius: 8px; border: 1px solid #e5e5e5; margin-bottom: 32px;">
        ...
        <div style="display: flex; gap: 16px; flex-wrap: wrap;">
            ...
            <div style="flex:1; min-width: 180px; background: #f3f6fa; border-radius: 8px; padding: 24px 12px; text-align: center; border: 1px solid #b3c6e0;">
                <div style="font-size: 2.5em; font-weight: bold; color: #0073aa;"><?php echo number_format($stats['unknown']); ?></div>
                <div style="font-size: 1.1em; color: #0073aa; margin-top: 8px;">Nieznane</div>
                <div style="font-size: 0.95em; color: #0073aa; margin-top: 4px;">
                    <?php echo $stats['total'] > 0 ? round(($stats['unknown'] / $stats['total']) * 100, 1) : 0; ?>%
                </div>
            </div>
        </div>
    </div>
