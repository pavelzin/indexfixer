...
define('INDEXFIXER_URL_LIMIT', 1000); // Maksymalna liczba URL-ów do sprawdzania
...
function indexfixer_check_urls() {
    ...
    $all_urls = IndexFixer_Fetch_URLs::get_all_urls();
    // DODAJ WSZYSTKIE BRAKUJĄCE URL-E DO BAZY (w tym custom post types)
    foreach ($all_urls as $url_data) {
        $existing = IndexFixer_Database::get_url_status($url_data['url']);
        if (!$existing) {
            IndexFixer_Logger::log('Dodaję do bazy: ' . $url_data['url'] . ' | typ: ' . ($url_data['post_type'] ?? 'brak'), 'info');
            IndexFixer_Database::save_url_status(0, $url_data['url'], array(
                'simple_status' => 'unknown',
                'post_type' => isset($url_data['post_type']) ? $url_data['post_type'] : null,
                'post_date' => isset($url_data['post_date']) ? $url_data['post_date'] : null
            ));
        } else {
            IndexFixer_Logger::log('Już istnieje w bazie: ' . $url_data['url'] . ' | typ: ' . ($url_data['post_type'] ?? 'brak'), 'info');
        }
    }
    $urls = array_slice($all_urls, 0, $url_limit);
...
class IndexFixer {
    ...
    public function __construct() {
        ...
        // Hooki aktywacji i deaktywacji
        register_activation_hook(__FILE__, array($this, 'activate'));
        register_deactivation_hook(__FILE__, array($this, 'deactivate'));
        
        // Cron dla odnawiania tokenów
        add_action('indexfixer_refresh_tokens', array($this, 'refresh_tokens_cron'));
        
        // Hook dla sprawdzania URL-ów w tle
        add_action('indexfixer_check_urls_event', array($this, 'check_urls_background'));
        
        // NOWY: Cron dla codziennego czyszczenia
        add_action('indexfixer_daily_cleanup', array($this, 'daily_cleanup_cron'));
    }
    
    public function init() {
        // Ładuj składniki wtyczki
        load_plugin_textdomain('indexfixer', false, dirname(plugin_basename(__FILE__)) . '/languages');
    }
    ...
    public function activate() {
        ...
        IndexFixer_Logger::log('Wtyczka IndexFixer została aktywowana', 'success');
    }
    
    public function deactivate() {
        // Usuń zaplanowane zadania
        wp_clear_scheduled_hook('indexfixer_refresh_tokens');
        wp_clear_scheduled_hook('indexfixer_daily_cleanup');
        
        IndexFixer_Logger::log('Wtyczka IndexFixer została deaktywowana', 'info');
    }
    
    public function refresh_tokens_cron() {
        if (class_exists('IndexFixer_Auth_Handler')) {
            $auth = new IndexFixer_Auth_Handler();
    ...
    public function check_urls_background() {
        // Sprawdź czy proces nie jest już uruchomiony
        $process_running = get_transient('indexfixer_process_running');
        if ($process_running) {
            IndexFixer_Logger::log('⏳ Proces sprawdzania jest już uruchomiony - pomijam', 'warning');
            return;
        }
        
        // Uruchom sprawdzanie
        $this->run_url_checking();
    }
    
    // NOWA FUNKCJA: Cron dla codziennego czyszczenia
    public function daily_cleanup_cron() {
        IndexFixer_Logger::log('🕐 Uruchomiono codzienne czyszczenie przez cron', 'info');
        
        if (class_exists('IndexFixer_Database')) {
            $cleaned = IndexFixer_Database::daily_cleanup();
            
            if ($cleaned > 0) {
                IndexFixer_Logger::log("✅ Codzienne czyszczenie: Usunięto $cleaned URL-ów", 'success');
            } else {
                IndexFixer_Logger::log('ℹ️ Codzienne czyszczenie: Brak URL-ów do usunięcia', 'info');
            }
        }
    }
    
    private function run_url_checking() {
        // Istniejący kod sprawdzania URL-ów...
        // (nie zmieniam istniejącej logiki)
    }
}

function indexfixer_admin_enqueue_scripts($hook) {
    if ('toplevel_page_indexfixer' !== $hook) {
        return;
    }
    
    wp_enqueue_style('indexfixer-admin', plugins_url('assets/css/admin.css', __FILE__));
    wp_enqueue_script('chart-js', 'https://cdn.jsdelivr.net/npm/chart.js', array(), '3.7.0', true);
    wp_enqueue_script('indexfixer-admin', plugins_url('assets/js/admin.js', __FILE__), array('jquery', 'chart-js'), '1.0.0', true);
}
add_action('admin_enqueue_scripts', 'indexfixer_admin_enqueue_scripts'); 